...
#include <util/delay.h> 	// for 10ms delay in copy scratchpad
...
uint8_t DS18X20_format_from_decicelsius( int16_t decicelsius, char str[], uint8_t n)
{
	...
	// range from -550:-55.0\B0C to 1250:+125.0\B0C -> min. 6+1 chars
...
uint8_t DS18X20_format_from_maxres( int32_t temperaturevalue, char str[], uint8_t n)
{
	...
	// range from -550000:-55.0000\B0C to 1250000:+125.0000\B0C -> min. 9+1 chars
